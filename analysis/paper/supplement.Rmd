...
The simple cost method for setting edge weights generally performed worse than the grade penalty and Hardy methods (compare Figures 2 and \@ref(fig:aspat-perf-fig-simp), and Figures 5 and \@ref(fig:spat-perf-fig-simp)). 
...
Execution time and memory requirements increase with the number of nodes in the graph, and the slope of that increase is highest for the MST method (Figure \@ref(fig:speed-fig)). Except on the very small landscape the ILCP method is faster than the MST method. Reducing the number of landings by targeting centroids also reduces execution time (Figure \@ref(fig:speed-fig)). The Hardy method is significantly faster than any of the R package methods.
...
Profiling with the `Rprof` function shows which parts of a function take the longest. On a test run with 1 million cells in the weight raster and 10 landings we profiled the performance of the ILCP and MST methods. Both methods begin by constructing a full graph, which took 3-5 s. For the ILCP method, iteratively determining the shortest paths is the only other step that takes more than a second to run, and this run time will increase with the number of landings (Figure \@ref(fig:profiling-ilcp)). For the MST method, calculating cost distances among landings and from each landing to the nearest point on the existing road distance takes the most time, which also increases with the number of landings (Figure \@ref(fig:profiling-mst)). Determining the locations of the paths in the minimum spanning tree also takes about 6 seconds. We experimented with using graph construction methods in the package `gdistance` [@van_etten_r_2017], but Lochhead et al's [-@lochhead_demo_2018; -@lochhead_linking_2022] method is faster (compare Figures \@ref(fig:profiling-ilcp) and \@ref(fig:profiling-ilcp-gdistance)).     

```{r profiling-ilcp, fig.cap="Profiling results for the ILCP method showing functions that take > 1 s to run on a cost raster with 1 million cells and 10 landings. Functions higher in the call stack are called by functions lower down.", fig.height=3}
if(!file.exists(here::here("analysis/data/derived_data/ilcp_100_Rprof.out"))){
  source(here::here("analysis/scripts/test_profile.R"), keep.source = TRUE)
}

profstat(here::here("analysis/data/derived_data/ilcp_100_Rprof.out"))
...
```{r profiling-mst, fig.cap="Profiling results for the MST method showing functions that take > 1 s to run on a cost raster with 1 million cells. Functions higher in the call stack are called by functions lower down.", fig.height=5}
profstat(here::here("analysis/data/derived_data/mst_100_Rprof.out"))
```

```{r profiling-ilcp-gdistance, fig.cap="Profiling results for the ILCP method using the gdistance package internally showing functions that take > 1 s to run on a cost raster with 1 million cells. Functions higher in the call stack are called by functions lower down.", fig.height=5}
profstat(here::here("analysis/data/derived_data/ilcp_gdistance_100_Rprof.out"))
```
