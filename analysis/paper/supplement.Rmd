...
Profiling with the `Rprof` function can reveal which parts of a function take the longest. On a test run with 1 million cells in the weight raster and 10 landings we profiled the performance of the ILCP and MST methods. Constructing the graph took ~ 4 s for both methods. For the ILCP method, the iteratively determining the shortest path is the only other step that takes more that a second to run. Each iteration is relatively fast but must be repeated for each landing. For the MST method, the longest running step is the building of the second graph which is used to determine the MST. Determining the shortest path to connect the parts of the MST also takes about 2 seconds.    
