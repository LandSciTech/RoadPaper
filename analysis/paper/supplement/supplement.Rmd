...
## Integrating roads with a spatial stochastic model

To demonstrate how the roads R package can be integrated with an existing spatial stochastic model we projected roads to accompany expected harvest blocks in the state-and-transition simulation models described in @daniel_incorporating_2017. These models are created using SyncroSim and we used the rsyncrosim package to interacte with the SyncroSim session from R [@daniel2025]. The the script "8_integrate_roads_stsim_model.R" demonstrates how this can be done and an updated version of the Caribou Forest SyncroSim library originally included in @daniel_incorporating_2017 is available in our [OSF repository](https://osf.io/8vmqh/?view_only=d168967961914f22af89e44c19aec019)

We projected roads for one iteration of the model with 10 timesteps using a 250 m resolution weight raster that resulted in 177555 nodes in the graph. Projecting roads took an average of 15 seconds per timestep and the whole script took 3.2 minutes to complete on a laptop with 16 cores, 64 GB of RAM and a 13th Gen Intel(R) Core(TM) i7-13850HX (2.10 GHz) processor. The weight raster was a very simple cost raster with water bodies set to a cost of 1000 and all other areas set to 1, except existing roads which were simply drawn as a diagonal line across the raster extent to enable the demonstration.

```{r integrating, fig.height=3, fig.width=fig_widths["single"], fig.cap= "Projected roads for the Caribou Forest Management Unit within the boreal forest region of the province of Ontario, Canada. The figures below show the harvest blocks for the current timestep in dark grey and the cumulative roads built in the simulation in red.", fig.show='hold', out.width="450px", fig.align='center'}
