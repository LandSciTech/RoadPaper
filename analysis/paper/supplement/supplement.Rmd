...
Corresponding author: Josie Hughes; [josie.hughes\@ec.gc.ca](mailto:josie.hughes@ec.gc.ca){.email}; Landscape Science and Technology Division, National Wildlife Research Center, Environment and Climate Change Canada, Ottawa, ON, Canada
...
In the simple cost method, we first calculate a slope raster $S$, then calculate an input cost raster $X$ that depends on slope $S$: \begin{equation}
   X = 
   \begin{cases}
   ...
   \end{cases}
\end{equation} The weight of an edge between two nodes is the mean value of the input cost raster at each of those nodes, adjusted for the euclidean distance between the nodes:
...
## Integrating roads with spatial stochastic forest change projection models

To demonstrate simple coupling of the roads R package with a spatial stochastic forest change projection model we created a road network to connect the harvest blocks in Daniel et al's [-@daniel_incorporating_2017] [SyncroSim](https://syncrosim.com/) state-and-transition simulation model for the Caribou Forest Management Unit in the boreal forest region of the province of Ontario, Canada. We updated the published model to work with current versions of the ST-Sim package (v 4.3.8) for SyncroSim (v 3.1.12), and used the [rsyncrosim v 2.1.2](https://cran.r-project.org/web/packages/rsyncrosim/index.html) package to extract harvest information from the SyncroSim model [@daniel_rsyncrosim_2025]. We provide code for this example in the ["8_integrate_roads_stsim_model.R"](https://github.com/LandSciTech/RoadPaper/blob/master/analysis/scripts/8_integrate_roads_stsim_model.R) script of our manuscript GitHub repository. The updated version of the Caribou Forest SyncroSim library is available in our [OSF repository](https://osf.io/8vmqh/?view_only=d168967961914f22af89e44c19aec019).

The computational efficiency of our example scenario is improved by doing some of the more computationally expensive graph construction work only at the first timestep. We projected roads for 10 timesteps of one model iteration using a 250 m resolution weight raster, giving 177555 nodes in the graph. Road projection took an average of 11 seconds per timestep, and the whole script took 3.13 minutes to run on a laptop with 16 cores, 64 GB of RAM and a 13th Gen Intel(R) Core(TM) i7-13850HX (2.10 GHz) processor. The weight raster was a very simple cost raster with water bodies set to a cost of 1000 and all other areas set to 1. For demonstration purposes, we created a simple diagonal line initial road. 

In some cases, users may wish to combine our road network projection methods with a model that specifies how roads (and the impacts of them) change over time. To demonstrate this possibility we include an option for decommissioning roads after a number of timesteps in our ["8_integrate_roads_stsim_model.R"](https://github.com/LandSciTech/RoadPaper/blob/master/analysis/scripts/8_integrate_roads_stsim_model.R) script. Note that the simulations are somewhat less efficient in this case (3.87 minutes, 14 seconds per timestep) in part because graph construction work is repeated at each timestep. 

In these example, the location of roads has no effect on subsequent harvest or other landscape change processes so the road projection need not be integrated directly into the SyncroSim model. Similarly loose coupling of our road network projection methods with outputs from other forest landscape change modeling frameworks [e.g. [@scheller_design_2007; @barros_empowering_2023] could be achieved simply by replacing the harvest outputs from the SyncroSim model with harvest outputs from another modeling framework.

In a more complex case, interactions between road network construction and other landscape change processes could be enabled by creating an [add-on package](https://docs.syncrosim.com/how_to_guides/package_addon.html) to the SyncroSim stsim base package, or a [SpaDES module](https://cran.r-project.org/web/packages/SpaDES.core/vignettes/ii-modules.html). Writing, documenting, publishing and supporting an stsim add-on package or a SpaDES module is beyond the scope of this project, but we note that much of the required code is included in our ["8_integrate_roads_stsim_model.R"](https://github.com/LandSciTech/RoadPaper/blob/master/analysis/scripts/8_integrate_roads_stsim_model.R) script.

```{r integrating, fig.height=3, fig.width=fig_widths["single"], fig.cap= "Projected roads for the Caribou Forest Management Unit without decommissioning. The figures below show the harvest blocks for the current timestep in dark grey and the cumulative roads built in the simulation in black.", fig.show='hold', out.width="450px", fig.align='center'}
knitr::include_graphics(here("analysis/figures/stsim_roads_dtNAtg_12545.it1.ts1.png"))
knitr::include_graphics(here("analysis/figures/stsim_roads_dtNAtg_12545.it1.ts5.png"))
knitr::include_graphics(here("analysis/figures/stsim_roads_dtNAtg_12545.it1.ts10.png"))
```

```{r integrating2, fig.height=3, fig.width=fig_widths["single"], fig.cap= "Projected roads for the Caribou Forest Management Unit with decommissioning after 5 timesteps of disuse. The figures below show the harvest blocks for the current timestep in dark grey. Road colour indicates timesteps since last use.", fig.show='hold', out.width="450px", fig.align='center'}
knitr::include_graphics(here("analysis/figures/stsim_roads_dt5tg_12545.it1.ts1.png"))
knitr::include_graphics(here("analysis/figures/stsim_roads_dt5tg_12545.it1.ts5.png"))
knitr::include_graphics(here("analysis/figures/stsim_roads_dt5tg_12545.it1.ts10.png"))
```
