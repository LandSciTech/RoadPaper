...
Simple methods for projecting low-cost routes from an existing road network to target locations (i.e. harvest cutblocks) include simple least cost paths (LCP), dynamic least cost paths (DLCP) and least cost path minimum spanning trees (MST) (Figure \@ref(fig:proj-meth-fig)). We use Dijkstra's shortest path algorithm [-@dijkstra_note_1959] to determine the least cost path between a target location (e.g. centroid of a cutblock) and the existing road network. Least cost paths to each cutblock are determined independently in the simple method (LCP). The dynamic least cost paths (DLCP) method evaluates the path to each cutblock in sequence so that earlier paths are considered in the cost for later paths. Sequence is determined by a heuristic, such as the distance to the nearest existing road in these examples. The minimum spanning tree (MST) method calculates the minimum cost network to connect all cutblocks to the road network using Kruskal's algorithm [-@kruskal_shortest_1956]. See Appendix A for a more detailed explanation of these methods. We focus our analysis on the DLCP and MST approaches which yield more realistic road networks (Figure \@ref(fig:proj-meth-fig)). We compared these methods to an alternative iterative least cost path method developed by Hardy [-@hardy_forest_2019;-@hardy_landis-ii_2023].
...
Despite a rapidly growing body of evidence of complex and varied impacts of road networks on wildlife and other values, and increasing use of spatial stochastic models for forecasting impacts of forest disturbance across regions, very few spatial stochastic projections of forest disturbance dynamics have included road network projections, in part because tools suited for this purpose have not been available. Hardy's Landis-II extension [-@hardy_forest_2019; -@hardy_landis-ii_2023] can be integrated with other components of the Landis-II modelling framework [e.g. @labadie_global_2023]. Our implementations of simple iterative least cost path and minimum spanning tree methods [@lochhead_demo_2018; @lochhead_linking_2022] in the open source R package `roads` [@endicott_roads_2023] provides a comparable alternative to Hardy's method that can be easily integrated into R workflows and modelling frameworks used for forecasting forest change in Canada and elsewhere [e.g. @barros_empowering_2023; @daniel_state-and-transition_2016].

We evaluated performance of these algorithms in a topographically complex region of British Columbia where we anticipated projection of forest road network patterns would be challenging, using a very simple cost surface model. Thus, we were not surprised to less accurate projections of road density and presence than Hardy found in less mountainous regions of eastern Canada with a model that more carefully accounted for water crossing costs [-@hardy_landis-ii_2023]. The stream crossing behaviour of road projection algorithms in Revelstoke could be improved with a more detailed high resolution cost surface, but none of these simple algorithms will project switchbacks on steep slopes that are characteristic of forest roads in mountains. A related fundamental limitation is that none of these algorithms can project road tortuosity within raster cells, so in complex landscapes projections of road density will become less accurate as raster cell resolution increases. The performance of these algorithms varies with terrain; in topographically complex landscape high resolution cost surfaces that account for the costs of stream and river crossings will address some but not all of the challenges of forest road projection in mountain regions. 

Aspatial metrics of overall projection accuracy can be difficult to interpret. To more deeply understand variation in the behaviour of road network projection algorithms we found it helpful to distinguish between performance within cutblocks and outside of them, and to examine spatial variation in discrepancies. Examination of spatial mismatch between observed and projected roads highlighted errors in our input data, and removing mapped cutblocks without observed access roads from our analysis helped resolve the apparently contradictory results. The lack of consistent accurate information on the location of resource roads across Canada and elsewhere [@poley_identifying_2022](Poley, ) Better harvest & existing road data would help errors in projections from errors in the input data (cite lit in coops paper)

```{=html}
<!-- Discussion pts
- we selected revelstoke because it is topographically complex, so a good challenge. Hardy result in eastern Canadian shield showed better overall accuracy projecting road density in less extreme terrain when the costs of water crossings are more carefully accounted for. Stream crossing behaviour in Revelstoke landscape could be improved with more a detailed high resolution cost surface. But our results highlight that performance of these algorithms depends not only on accounting for cost but on complexity of the terrain. None of these simple algorithms will project switchbacks on steep slopes.  
- Aspatial overall performance metrics can be difficult to interpret. To more deeply understand variation in projection accuracy we found it helpful to distinguish between performance within cutblocks and outside of them, and to examine spatial variation in discrepancies. Examination of spatial mismatch clarified that some discrepancies in our landscape are due to errors in our input data, rather than projection errors, and removing mapped cutblocks without observed access roads from our analysis helped resolve the apparently contradictory results. Better harvest & existing road data would allow us to more distinguish errors in projections from errors in the input data (cite lit in coops paper)
 - Outside cutblocks, we saw very little variation in performance among our method variants - any of them are somewhat better than nothing, but they don't make a big differene, partly because in this example projected new roads over fairly short distances, and new landings were always fairly close to existing roads or other landings. Would be interesting to test in a context where targets are further from existing roads, but outside cutblocks our DLCP algorithm is very similar to Hardy's algorithm, so expect similar performance. Within cutblocks that also have observed roads, the density of landing points has a much larger impact on performance than the choice of DCLP or MST algorthm. Hardy algorithm most accurately projected road density within blocks - could up density of landing pts in DCLP/MST algorithms to match this. But examination of spatial patterns clarifies that algorithms can get the right density without replicating the pattern or mechanism. Hardy density is high in part because the algorithm is prefering turns over straigt lines straight lines, which may be an artefact of searching for nearest destinations >200 m away on a 100 m grid. Projected wiggles are quite different from observed switchbacks. 
- Focussed here on forest resource roads. Note the algorithms could also work in other situations where the objectives of road building are to access target locations and minimize cost. Not a useful algorithm for projecting linear features that were constructed for some other purpose such as seismic testing (e.g. Figure \@ref(fig:ftNelson)), or for circumstances where information about target locations is not available. 
- If realistic switchbacks in mountainous terrain is a key goal then none of these algorithms will do. But before investing in more complexity recognize that adequacy depends on the goal. For some purposes, projections from relatively crude cost models and simple algorithms may be adequate. For example, models of caribou movement often include distance to roads as a predictor, and at a landscape scale a combined metric of buffered anthropogenic disturbance best explains variation in boreal caribou demographic rates (). In our example landscape, projections of size disturbance footprints (500 m and 1km buffer) and distance to road are pretty good (and better than nothing). For wildlife response models informed by distance to road () or buffered disturbance footprint (), somewhat crude road projections are likely good enough. When constructing new wildlife response models intended for projection, it may be helpful to select these more easily projectable (Bodner) predictors.
- There are a bewildering array of possible metrics of connectivity and fragmentation we did not consider in this paper. In general, performance will vary among metrics, and it is difficult determine what is good enough without reference to particular circumstances (objectives, species, landscapes). Rather than expanding the array of road performance metrics considered, we believe our efforts are better spent making road projection tools easy to use, to enable users devise performance tests appropriate to their particular circumstances and goals. The best performance metrics will be those most closely linked to the outcome of interest. For example, where the outcome of interest is space use or connectivity projected from more (e.g. Whittington, Labadie) or less (samc, pither etc) species-specific movement models consider testing sensitivity of these models to the difference between observed and retrospectively projected roads. We hope R package and open workflow associated with this paper can help enable retrospective projection and relevant performance assessment in other contexts. However, we also recognize there is work associated with acquiring data to construct cost surfaces & retrospectively project roads. For those seeking a lower effort option, we provide our output maps and an example script () to enable calculation of alternative performance metrics of interest for our Revelstoke example.
