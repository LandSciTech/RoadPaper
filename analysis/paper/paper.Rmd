...
# One option is removing extreme value in distance to road
aspat_perf$plt$data <- aspat_perf$plt$data %>% mutate(prop_dif = ifelse(metric == "Distance\nto road" & sampleDens == "Cutblocks only", NA, prop_dif))
...
aspat_perf$plt+coord_flip(ylim = c(-0.35, 0.35))+
  geom_text(aes(label = lab), data = ann_plus, hjust = "inward", vjust = 0.2,
