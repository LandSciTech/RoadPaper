...
We chose the Revelstoke timber supply area (TSA 27), in British Columbia, Canada as an assessment area because it is a mountainous areas with approximately 55% of non-forested land where road building is difficult or impossible (e.g. alpine, lakes, swamp, brush, rock) [@hachey_revelstoke_2010]. Thus, the area provides opportunities to observe the responses of road projection algorithms to steep complex terrain. Forestry is the dominant resource extraction activity in this region and much of the road network was built to access forest harvest cutblocks (Figure \@ref(fig:study-area)), which allows projection of the forest road network from historic forest harvest data.  Revelstoke is in the interior-wet belt of the province where forests are dominated by Englemann spruce (31%), Hemlock (23%), Cedar (22%), and Douglas-fir (18%) [@ministry_of_forests_revelstoke_2021]. The area is home to wildlife of concern including including grizzly bears and Southern Mountain woodland caribou [@hachey_revelstoke_2010]. Eight herds of Southern Mountain caribou have been extirpated or likely extirpated, and remaining herds are considered Endangered by COSEWIC and listed as threatened under the Canadian Species at Risk Act [@palm_long_2020]. Mountain caribou avoid roads leading to a loss of available habitat [@polfus_identifying_2011; @apps_factors_2006], suffer increased mortality near roads [@apps_spatial_2013], and decline in abundance as the footprint of roads and cutblocks increases [@lochhead_linking_2022]. Roads also alter grizzly habitat use, home range selection, movements, population fragmentation, survival, reproductive rates, population density, trends, and conservation status [@proctor_effects_2020].
...
All data sets were accessed using the `bcdata` R package and filtered to the extent of the Revelstoke Timber Supply Area [@teucher_bcdata_2021]. These data sets are continuously updated so we have stored the versions used in our analysis in an Open Science Framework repository <https://osf.io/8vmqh/>. We were not granted permission to share the DRA data set so it must be accessed from <https://catalogue.data.gov.bc.ca/dataset/bb060417-b6e6-4548-b837-f9060d94743e>. Scripts used for data acquisition and analysis associated with this paper are available on GitHub <https://github.com/LandSciTech/RoadPaper>.  

<!-- TO DO: number analysis scripts to clarify workflow -->

## Road Projection Methods and Variants

We focus on methods that link multiple target locations to an existing road network for strategic purposes, described by Dean (1997) as the multiple target access problem (MTAP), where the objective is to find the minimum-cost locations for new roadways that link landings within harvest cutblocks (i.e., log decks) to the existing road network. We implemented four methods for solving the MTAP in the roads package, representing a gradient of computational complexity and operational detail; in this manuscript we focus on comparing the dynamic least cost path (DLCP) and minimum spanning tree approaches (MST) to an alternative iterative least cost path method developed by Hardy [-@hardy_forest_2019;-@hardy_landis-ii_2023] (Figure \@ref(fig:proj-meth-fig)).

The DLCP approach solves the MTAP by decomposing the problem into several smaller single target access problems (STAP). For each STAP, a least cost path from a single source to sink is solved using Dijkstra's shortest path algorithm [-@dijkstra_note_1959]. The algorithm is parameterized with a weighted graph wherein each node corresponds to a raster pixel representing one of three components of the potential forest road network: a landing, a point on an existing road, or an intermediate point that is a plausible location for road construction. Each node is connected with up to eight neighbours, and the cost of construction along edges is weighted by the distance to neighbouring nodes and the average of the construction cost at the source and destination nodes (Table \@ref(tab:cost-tbl)). The underlying grid pattern in these methods does not support curvature or switchbacks required in operational road design. The DLCP approach solves each STAP sequentially, starting with target that is nearest to an existing road. The existing road network and construction cost surface is updated with each new road segment before moving on to the next nearest target. The method was first developed by Anderson and Nelson (2004) who noted that the projected network is sensitive to the ordering of the STAP and recommended a heuristic for connecting the landings and finding optimal branch locations to deal with this sensitivity. 

The MST approach extends the LCP approach by providing a heuristic for linking landings or evaluating road branching opportunities. Dean (1997) showed that heuristics with branch evaluation were able to reproduce 80% of the observed road locations in their study area. The first step in our MST approach is to estimate the least cost paths that link each of the landings and the least cost paths that link each landing to the existing road network. These results are used to create a weighted graph wherein nodes are either landings or points on the existing road network, and edge weights correspond to the total cost of the least cost path between pairs of nodes. The minimum spanning tree for this graph is solved using Kruskal's algorithm [-@kruskal_shortest_1956], which links every node in graph with a tree that minimizes edge weights and does not include loops. This method differs from the minimum spanning tree method used by Clark et al. (2000) who used Euclidean distances rather than least costs for edge weights.
...
To help make these road network projection methods more easily and widely useable, we implemented the DLCP and MST methods in the open source R package `roads` [@endicott_roads_2023]. The `roads` package is dependent on several other R packages that made this work possible, including, among others: `igraph` for calculating least cost paths and minimum spanning trees [@csardi_igraph_2006; @noauthor_igraph_2022], `terra` for manipulating spatial raster data [@hijmans_terra_2023], and `sf` for manipulating spatial vector data [@pebesma_simple_2018; @pebesma_sf_2022]. The package is available on CRAN (The Comprehensive R Archive Network). The source code for the package is available on GitHub (<https://github.com/LandSciTech/roads>), along with tutorials and documentation (<https://landscitech.github.io/roads/>).
...
TO DO: Sarah are these statements still correct? Please revise as necessary.
...
```{r aspat-perf-fig, fig.height=4, fig.width=fig_widths["two"], fig.cap="Proportional difference of projected mean from observed mean road metrics using different projection method variants with a 1 ha resolution cost raster. Metrics were summarised within cutblocks, outside cutblocks, and overall. Values greater than 0 indicate overprojection of the metric and negative values indicate an underprojection. Cutblocks that did not intersect the observed road network were excluded"}
...
dlcp_time <- meanTableRealCuts %>% 
  filter(method == "dlcp", areaMean == "overall") %>% 
...
Various optimization and simulation techniques have been used for projecting forestry roads into the future and across landscapes, however, we focused on the simulation methods that link multiple harvest units (a.k.a cutblocks) to an existing road network for strategic purposes. Dean (1997) described this road projection problem as the multiple target access problem (MTAP), where the objective is to find the minimum-cost locations for new roadways that link forested stands or landings (i.e., log deck) to the existing road network. We implemented four methods for solving the MTAP in the roads package, representing a gradient of computational complexity and operational detail, starting with the least complicated or the snapping approach, followed by independent and dynamic least cost paths (LCP, DLCP) and finally, the minimum spanning tree approach (MST). 

The LCP and DLCP approaches solve the MTAP by decomposing the problem into several smaller problems called single target access problems (STAP). For each STAP, a least cost path from a single source to sink is solved using Dijkstra's shortest path algorithm (1959) which minimizes the distance between a landing and the current road network, subject to costs or penalties incurred from road construction and travel. A mathematical weighted graph consisting of nodes and edges is used to parameterize Dijkstra's algorithm wherein each node corresponds to a raster pixel which may be any one of three attributes in the forest road network: (i) a landing, (ii) a point on an existing road, or (iii) an intermediate point that is a plausible location for road construction. Each node is connected with up to eight neighbours, and the cost of construction along each edge is weighted by the distance to the neighbouring node and the average of the construction cost at the source and destination nodes (or pixels) (Table \@ref(tab:cost-tbl)). The underlying grid pattern in these methods does not support curvature or switchbacks required in operational road design. 

The LCP and DLCP approaches solve each STAP sequentially, starting with target that is nearest to an existing road. In the DLCP algorithm, the existing road network and construction cost surface is updated with each new road segment before moving on to the next nearest target, whereas the LCP algorithm builds a connection to each target independently. The DLCP method was first developed by Anderson and Nelson (2004) and provides a feasible solution to the MTAP but the projected network is sensitive to the ordering of the STAP. They recommended a heuristic for connecting the landings and finding optimal branch locations to deal with this sensitivity. 

The MST approach extends the LCP approach by providing a heuristic for linking landings or evaluating road branching opportunities. Dean (1997) showed that heuristics with branch evaluation were able to reproduce 80% of the observed road locations in their study area. Our MST approach consists of three steps for evaluating branch locations while projecting roads: 1) estimate the least cost paths that link each of the landings and the least cost paths that link each landing to the existing road network, 2) create a weighted graph where each node is either a landing or a point on the existing road network and edge weights correspond to the total cost of the least cost path between pairs of nodes from the previous step, and 3) solve the minimum spanning tree for this graph. The minimum spanning tree is solved using Kruskal's algorithm (1956) where every node in the graph is linked to form a tree such that the sum of edge weights is minimized, and the links do not form a loop. Clark et al. (2000) also used a minimum spanning tree to project simple road networks during harvest scheduling, but they used Euclidean distance rather than least cost paths for edge weights.

TO DO: add references to appendix.
