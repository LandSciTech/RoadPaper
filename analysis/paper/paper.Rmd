...
TO DO: talk about key differences from Hardy algorithm.
TO DO: talk about implications of algorithms for speed/computational cost.
...
[a]This and a few other spots is a lot of citations for the same fact. I have been told before not more that 3. It would help reduce overall refs to remove any that are not also used else where. If you want to be clear that it is not an exhaustive list I have seen others use (E.g Ref 1, Ref2)
[b]This feels a bit backwards since road projection for harvesting is it's own thing not really something that needs to be highlighted by a wildlife paper example. I would restructure more as "While road projection tools are not typically used in forest landscape models, there are existing tools for operational forest road planning and design. These have occasionally been incorporated into forest landscape models (Leston et al) but there are x reasons why this is not ideal. " and then carry on about promising options
[c]I feel like the concept of being "adequately accurate" is fairly key to the whole paper and should be defined and explained in the previous paragraph in relation to why the operational tools are not as useful for landscape modelling.
