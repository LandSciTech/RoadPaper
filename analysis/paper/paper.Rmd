...
tsaCost <- raster(here(data_path_drvd, "testing_cost.tif"))
...
dlcpProj <- projectAll(tsbs = tsb,
                      paramTable = paramTable %>% filter(sampleType == "regular"),
                      costSurface = tsaCost,
                      cutblocks = cutblocks,
                      existingRoads = exRoads,
                      fileLocation = paste0(data_path_drvd, "for_fig/dlcp"),
                      method = "dlcp")
...
# organize data for ggplot
meanTable_long <- meanTable %>%
  pivot_longer(-c(sampleType, sampleDens, areaMean, resolution),
               names_to = "metric", values_to = "response")%>%
  mutate(sampleType = ifelse(sampleType == sampleDens, NA_character_, 
                             sampleType),
         sampleDens = paste(sampleType, sampleDens) %>% 
