---
title: "Assessment and implementation of resource road simulation methods for projection of cumulative effects on wildlife"
author:
  - name: Josie Hughes
...
We chose the Revelstoke timber supply area (TSA 27), in British Columbia, Canada as an assessment area because forestry is the dominant resource extraction activity in this region and much of the road network was built to access to forest harvest cutblocks (Figure 1). This allows projection of the forest road network from historic forest harvest data. Approximately 55% of the area is non-forested land where road building is difficult or impossible (e.g. alpine, lakes, swamp, brush, rock) [@brown_revelstoke_2010], so the area provides opportunities to observed the responses of road projection algorithms to barriers. Revelstoke is in the interior-wet belt of the province where forests are dominated by Englemann spruce (31%), Hemlock (23%), Cedar (22%), and Douglas-fir (18%) [@ministry_of_forests_revelstoke_2021]. The area contains wildlife species that are sensitive to roads (refs) including endangered southern mountain woodland caribou (refs).
...
All data sets were accessed using the `bcdata` R package and filtered to the extent of the Revelstoke TSA [@teucher_bcdata_2021].

<!-- TO DO: add links to repositories and explain what data is and is not there, and why -->

## Road Projection Methods and Variants

Simple methods for projecting low-cost routes from an existing road network to target locations (i.e. harvest cutblocks) include simple least cost paths (LCP), dynamic least cost paths (DLCP) and least cost path minimum spanning trees (MST) \@ref(fig:fig2)). We use Dijkstra's shortest path algorithm [-@dijkstra_note_1959] to determine the least cost path between a target location (e.g. centroid of a cutblock) and the existing road network. Least cost paths to each cutblock is determined independently in simple method (LCP). The dynamic least cost paths (DLCP) method evaluates the path to each cutblock in sequence so that earlier paths are considered in the cost for later paths. Sequence is determined by a heuristic such as the distance to the nearest existing road in these examples. The minimum spanning tree (MST) method calculates the minimum cost network to connecting all cutblocks to the road network using Kruskal's algorithm [-@kruskal_shortest_1956]. See Appendix A for a more detailed explanation of these methods. We focus our analysis on the DLCP and MST approaches which yield more realistic road networks (Figure \@ref(fig:fig2)). We compared these methods to an alternative iterative least cost path method developed by Hardy [ref].  
...
- as expected, not great at projecting exact road locations. sources of discrepancy include switchback, stream crossings, inaccurate harvest locations. All of these could be improved with more detailed high resolution cost & harvest data.
- density of landing pts has a larger effect than choice of road projection algorithm. So maybe don't get too worked up about the differences between one algorithm and another. if more accurate projections are required, focus first on refining cost & harvest landing locations.
- Selected revelstoke because it is topographically complex, so a good challenge. However, This example projected new roads over fairly short distances, new landings were always fairly close to existing roads or other landings so there was not a lot of space for the projection to go wrong. The results would likely be less accurate if the landings were spread further apart. But also less topographically complex landscapes may be simpler to project.
- For some purposes, projections from relatively crude cost models may be adequate. For this example landscape projections of disturbance footprints and distance to road are pretty good (and better than nothing). Density is harder to project partly because none of these algorithms can project variation in density within 100 m pixels. If projection of impacts on wildlife is a goal, it may be helpful to consider including more easily projectable road predictors in wildlife response models (Bodner,). If interested in predictors that we did not consider, see example script. Can evaluate alternative metrics without rerunning the projections.
-->
