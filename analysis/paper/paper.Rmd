---
title: "Assessment and implementation of forestry road network projection methods"
author:
  - name: Josie Hughes
...
The impacts of roads on wildlife are complex and varied. Mortality risk and stress on or near roads can be elevated due to vehicle collisions, hunting pressure, predation, noise and other stressors, but roads can also provide food, facilitate movement, and provide refuge [@hill_a_2021;@trombulak_review_2000;@fahrig_effects_2009].    
In Canadian forests, there is ample evidence of linear features attracting predators such as black bears and wolves seeking food or easy movement, while repelling prey such as caribou and moose [@dickie_corridors_2020;@polfus_identifying_2011;@mumma_predation_2018;@beauchesne_disentangling_2013;@leblond_avoidance_2013;@laurian_interactions_2012]; there is also evidence of predators avoiding features heavily used by humans [@muhly_human_2011;@whittington_towns_2022]. Changes in mortality risks, habitat quality, and connectivity associated with anthropogenic disturbance can in turn alter movement patterns [@tucker_moving_2018], demography and population viability [@johnson_science_2020; @decesare_linking_2014; @barrientos_the_2021;@apps_spatial_2013;@pinard_calving_2012;@fahrig_effects_2009]. Roads can also facilitate spread of invasive species [], and enable other development activities [@johnson_growth-inducing_2020].

Increasingly rapid environmental change, improvements in available cyberinfrastructure, and development of open source data, tools and methods have contributed to growing interest in ecological forecasting models [bodner, dietze, perfict]. Recent efforts to forecast impacts of climate change and anthropogenic disturbance on Canadian forests and forest-dwelling wildlife [@tremblay_harvesting_2018;@snell_importance_2018;@daniel_incorporating_2017;@stewart_;@michelletti_;recent quebec examples] build on a rich history of spatial-stochastic models of forest growth and succession following disturbance [landis;overview;stms;seles]. Despite the importance of roads for wildlife, none of these efforts have included forecasts of the development of road networks associated with forest harvest, largely because tools to project road network development over large spatial scales are not available.

Forest roads are also essential for harvest planning. Some forest management decision support systems include forest road projection methods [patchworks], but methods implemented in proprietary closed-source commercial software cannot be used in other contexts. Published methods intended to support strategic planning [@Anderson_projecting_2004;Clark et al., 2000; Dean, 1997] require inputs that are not publicly available, and employ algorithms that are too complex and detailed for application at regional, provincial or national scales. More promising options for projecting forest road network development at large scales include an iterative least cost path method developed by Clement [2021], and a least cost path minimum spanning tree algorithm used to project roads and reconstruct development history in British Columbia [lochhead published and not]. The former method is available as QGIS and Landis plugins, and the latter is not easily extricated from a larger open source modelling project that is specific to British Columbia []. Neither method has not been formally assessed, compared, published, or implemented in a way that allows for easy integration into the variety of modelling frameworks used for forecasting forest change in Canada and elsewhere [spades/syncrosim/landis].

Comment on metrics relevant to wildlife and what exactly accuracy means. 

In this work, we address the need for adequately accurate forest road network projection tools that are also widely and easily applicable by implementing simple iterative least cost path and minimum spanning tree methods in an open source R package. We compare the ability of these methods to reconstruct observed forest road network development in a mountainous region of British Columbia using performance metrics relevant to wildlife.
...
To spatially define the cost of building and traveling on a forestry road we developed a simple cost raster based on the slope and major barriers. Following the Interior Appraisal Manual of BC, the base cost per kilometer of road was a linear function of slope (Table 1) [@governmentofbritishcolumbia2021a]. Major water bodies were considered barriers and assigned a very high cost. Lastly, existing roads were assigned a cost of 0. The cost raster produced consists of 1703936 pixels with a resolution of 100 meters by 100 meters.

Table 1. Definitions of road construction costs used to build the cost raster.
...
We developed the `roads` R package to project where new roads will be built based on the existing road network, locations of expected resource extraction (i.e. harvest cutblocks) and the cost of traversing the landscape connecting the two. There are several different methods available for solving this problem, including snapping, least cost paths, dynamic least cost paths and minimum spanning trees (Figure \@ref(fig:fig2)). Snapping follows a straight line between the existing road and each cutblock regardless of the intervening landscape. The least cost paths (LCP) method uses Dijkstra's shortest path algorithm [-@dijkstra1959] to determine the optimal path between a cutblock and the existing road based on the road building cost between the two. The least cost path to each cutblock is determined independently of the other cutblocks. The dynamic least cost paths (DLCP) method uses the same LCP algorithm but evaluates the path to each cutblock in sequence so that earlier paths are considered in the cost for later paths. The sequence is determined by a heuristic such as the distance to the existing road. Finally, the minimum spanning tree (MST) method builds on the LCP method by determining the least cost path between all cutblocks and the road and between each cutblock. Then the minimum spanning tree connecting all cutblocks to the road is solved using Kruskal's algorithm [-@kruskal1956]. (See Appendix X for a more detailed explanation of these methods). In the remainder of this paper, we focus on the DLCP and MST approaches which are better aligned with how road networks are designed (Figure \@ref(fig:fig2)). In addition to the projection methods included in the `roads` package, we also applied the Forest Road Network Plugin for QGIS by Clément Hardy (<https://github.com/Klemet/ForestRoadNetworkPluginForQGIS>), hereafter referred to as the Hardy QGIS method. This plugin uses a similar DLCP algorithm to the `roads` package and uses the distance to the closest roads to order the cutblocks. Finally, in some cases the impact of forestry on wildlife is projected without considering the road development that will accompany it, we refer to this as the cutblocks only projection method and compare it to projections that do consider roads.
...
In order to assess the performance of the projections, the following characteristics of the projected and observed road networks were calculated and compared: 1) Road presence, whether or not roads are present in each pixel; 2) Road density, density of roads in meters per hectare (m/ha); 3) Distance to nearest road, distance from each raster cell to the nearest road. 4) Road disturbance footprint, a 1 km buffer around roads [@ibisch2016a]; 5) Forestry disturbance footprint, a 500 m buffer around roads and cutblocks. The forestry disturbance footprint follows the anthropogenic disturbance metric used for boreal caribou [@johnson2020], and was calculated using the `caribouMetrics` R package [@hughes]. Road density is an important metric that is often used when studying anthropogenic impacts on wildlife [@beazley2004]. Road density was calculated using the rasterizeLineDensity function within the `caribouMetrics` package. Distance to nearest road was calculated with a fast moving window method using the getDistFromSource function of the `roads` package. Only the road and forestry disturbance footprints were calculated for the cutblocks only method since the other characteristics are not applicable with out roads. All road characteristics were stored in raster format.

The aspatial performance of the projections was assessed by calculating the mean values of each road network characteristic and comparing the projected and observed values. This was done across the TSA and at the cutblock level, where road network characteristic rasters were masked by the cutblock polygons and the means were taken only from within cutblocks.

The projections were assessed for their spatial performance in two different ways. First, difference maps were created by subtracting the projected distance to the nearest road from the observed in order to highlight discrepancies between the road networks. The observed and projected road networks were then overlain on the difference maps and regions of major discrepancies were investigated using Google satellite imagery in QGIS. Secondly, the performance of each projection for the three binary characteristics (road presence, road disturbance footprint, and forestry disturbance footprint) was assessed. All pixels in the projected rasters fell into 5 categories when compared with the observed raster: 1) pre-existing roads; 2) agree roadless, both projected and observed show no roads; 3) agree roaded, both projected and observed show new roads; 4) false positive, the projection showed a road but no roads were observed; and 5) false negative, the projection showed no roads but a road was observed. These categories were summarized for each projection and characteristic and used to calculate the performance metrics sensitivity, precision and F-measure [@lee2021]. Sensitivity, also known as recall, gives the proportion of the true positives that were projected, while precision gives the proportion of positives projected that are true. Sensitivity will be lower when observed roads are missed and precision will be lower when roads are projected that were not observed. F-measure attempts to balance sensitivity and precision to give an overall score.

# Results

## Aspatial performance

For each projection method variant the aspatial performance varied across the 5 road network characteristics and between the within cutblock and overall scales (Figure \@ref(fig:fig3)). The buffering based characteristics, the road and forestry disturbance footprints, performed best within cutblocks, while the other characteristics, presence, density and distance to nearest road, performed similarly or better across the whole landscape. Across the whole landscape the road and forestry disturbance footprints and distance to nearest road characteristics were the closest to the observed mean, while the road presence and density characteristics were underestimated. With the coarser resolution cost raster road projections had mean road presence that was closer to the observed, while road density was further from the observed mean and distance to the nearest road was further from the observed mean inside cutblocks but closer for the landscape overall (Figure A-\@ref(fig:figA1)).

When comparing between projection method variants we found that the density of landing points chosen within cutblocks affected the results, with the high density method underestimating the road density and road presence metrics less than the low density and centroid methods. The sampling method for choosing points in the cutblocks, either random or regular, did not have a large effect on the results. Hardy's QGIS plugin was closest to the observed mean for the road presence and density characteristics but was among the furthest from the observed mean for the distance to the nearest road and road disturbance footprint characteristic. The cutblocks only method showed that not including road development underestimates anthropogenic disturbance. 
...
```{r fig3, fig.height=3, fig.width=fig_widths["two"], fig.cap="Proportional difference of projected mean from observed mean road characteristics using different projection method variations with a 100 m resolution cost raster across the entire Revelstoke TSA (“overall”) and within cutblocks (“cutblocks”). Values greater than 0 indicate the projection overestimated the characteristic and negative values indicate an underestimation. The cublocks only method is not defined for the road density, road presence or distance to road characteristics."}
...
Using difference maps to highlight areas of poor projections we found several potential causes of discrepancy. In most areas the projections remained within a reasonable vicinity of the observed roads and also realistically remained within valley bottoms without cutting across natural barriers such as mountains (Figure \@ref(fig:fig4)a). However, the cost raster was not detailed enough to reflect the costs of crossing streams or wet areas in valley bottoms and so the projections cross the valley more than the observed roads, which typically have two parallel roads on either side of the valley. There were also instances where cutblocks included in the data set appear to be misidentified and are not accessed by the observed road network (Figure \@ref(fig:fig4)b). In addition, the projected roads were not able to match the switchback pattern of observed roads in steep terrain (Figure \@ref(fig:fig4)c). 
...
As expected all the projection method variants preformed worst at projecting the road presence characteristic (F-Measure: `r perf_tbl_rng["Road presence"]`), while the forestry disturbance footprint had the best performance (F-Measure: `r perf_tbl_rng["Forestry disturbance\nfootprint"]`) and road disturbance footprint only slightly worse (F-Measure: `r perf_tbl_rng["Road disturbance\nfootprint"]`). The cutblocks only method performed worst for all the road network characteristics (Figure \@ref(fig:fig5)). For the road presence metric higher sampling densities led to somewhat higher F-measure values caused by fewer false negatives and only slightly more false positives. For the forestry and road disturbance footprint metrics, the F-measure was similar across projection method variants, while sensitivity was higher at high sampling densities and precision was higher at low sampling densities. The DLCP projection method had slightly lower performance than the MST projection method with the same sampling strategy. The DLCP method was faster to compute however, with a run time of `r dlcp_time` minutes compared to `r mst_time` minutes for the MST method.



# Discussion

**NOTE David's manuscript rough draft document includes other comments and bullets that might be relevant to rewriting the Intro and Discussion **

## Sarah's thoughts

- The projections work reasonably well for strategic planning purposes, especially if we are not concerned with roads inside cutblocks.

- The differences between algorithms were fairly small and it would be more valuable to focus on the accuracy of input data such as costs and cutblock locations rather than fine tuning the algorithms.

- This example projected new roads over fairly short distances, new landings were always fairly close to existing roads or other landings so there was not a lot of space for the projection to go wrong. The results would likely be less accurate if the landings were spread further apart.
