...
```{r study-area, fig.cap="Road construction costs according to a simple model informed by slope and major barriers (Table \@ref(tab:cost-tbl)) in the Revelstoke timber supply area in British Colombia, Canada.", cache=TRUE}
...
```{r proj-meth-fig, fig.cap="Comparison of minimum spanning tree (MST), dynamic least cost paths (DLCP), and the Hardy QGIS plugin road network projection algorithms. Landing locations were selected as the centroid of the cutblock, or a random or regularly spaced sample of points inside each cutblock (Table 2). Target cutblocks are outlined in grey, existing roads are black, projected roads are yellow, observed roads are red. Cost (1 ha resolution) is indicated by grey shading. The large four sided cutblock in the top right was added to more clearly show algorithm behaviour within cutblocks."}
