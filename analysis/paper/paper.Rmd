...
bench_res %>% filter(cutblocks_real == "revelstoke_real", sampleDens == 1e-05,
                     sampleType == "regular") %>% 
  pivot_longer(c(Elapsed_Time_sec, Peak_RAM_Used_MiB), names_to = "variable", 
               values_to = "values") %>% 
  ggplot(aes(y = values, x = toupper(method), fill = as.factor(resolution)))+
  geom_col(position = position_dodge())+
  scale_fill_discrete(type = met.brewer(pal_nm, 3))+
  facet_wrap(~variable, nrow = 2, scales = "free", 
             labeller = labeller(variable = c(Elapsed_Time_sec = "Time (s)", 
                                              Peak_RAM_Used_MiB = "Peak RAM used (MiB)"))) +
  labs(y = NULL, x = "Projection method", fill = "Resolution")
...
Although the MST method performed better than alternatives according to several metrics (Figure \@ref(fig:aspat-perf-fig)), it also requires more memory and more time to compute on large landscapes (Supplement D). All of these methods require construction of a large graph and therefore have substantial memory requirements; graph size depends on the number of pixels in the cost surface where road construction is possible (i.e. not NA), so options such as increasing raster resolution or constraining the portion of the landscape where roads are plausible can help reduce memory requirements. Memory requirements and computation time can also be reduced by setting a smaller number of landing targets (e.g. only target the centroid of each cutblock). Hardy comparison....
