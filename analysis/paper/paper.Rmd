...
```{r proj-meth-fig, fig.cap="We compared minimum spanning tree (MST), dynamic least cost paths (DLCP) and least cost path (LCP) road network projection algorithms. Landing locations were selected as the centroid of the cutblock, or a random or regularly spaced sample of points inside each cutblock (Table 2). Target cutblocks are outlined in grey, existing roads are black, projected roads are yellow, observed roads are red. Cost (1 ha resolution) is indicated by grey shading."}
...
Difference maps highlight several potential causes of discrepancy between our projections and the observed road network. In most areas the projections remained within a reasonable vicinity of the observed roads and also realistically remained within valley bottoms without cutting across natural barriers such as mountains (Figure \@ref(fig:proj-map-fig)a). However, the cost raster is not sufficiently details to reflect costs of crossing rivers, streams or other wet areas in valley bottoms so the projections cross valleys more than the observed roads which often run parallel to one another on either side of valleys. There were also instances where cutblocks included in the data set appear to be misidentified and are not accessed by the observed road network (Figure \@ref(fig:proj-map-fig)b). In addition, the projected roads were not able to match the switchback pattern of observed roads in steep terrain (Figure \@ref(fig:proj-map-fig)c). All these discrepancies could be addressed with more accurate harvest and cost information. 
