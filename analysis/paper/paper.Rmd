...
library(tmap)
library(sf)
library(waffle) # Used github version 1.0.1
...
meanTable_long <- meanTable %>%
  ...
  mutate(sampleDens = factor(sampleDens,
                             levels = c("centroid",
                                        ...
                                        "high sample density",
                                        "klementQGIS"),
                             labels = c("Centroid", "Low density", "High density",
                                        "Hardy QGIS")),
         ...
         sampleType = factor(sampleType,
                             levels = c("centroid",
                                        "random",
                                        "regular",
                                        "klementQGIS", "observed"),
                             labels = c("Centroid", "Random", "Regular",
                                        "Hardy QGIS", "Observed")))
...
prop_dif %>% 
  ggplot(aes(x = sampleDens, prop_dif, fill = sampleType))+
  geom_hline(aes(yintercept = 0), color = "grey")+
  geom_col(position = position_dodge2(preserve = "single",
                                      width = 0.75))+
  facet_grid(areaMean~ metric)+
  theme_classic()+
  scale_fill_manual(values = met.brewer(pal_nm, 4), name = "Sampling\nMethod")+
...
perf_tbl %>% 
  pivot_longer(c(sensitivity, precision, F_measure), names_to = "perf_meas",
               values_to = "value") %>% 
  ggplot(aes(sampleDens, value))+
  geom_col()+
  facet_grid(perf_meas~metric)+
  coord_flip()+
  ylim(0, 1)+
  theme_classic()
