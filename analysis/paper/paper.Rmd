...
All data sets were accessed using the `bcdata` R package and filtered to the extent of the Revelstoke Timber Supply Area [@teucher_bcdata_2021].

<!-- TO DO: add links to repositories and explain what data is and is not there, and why -->

## Road Projection Methods and Variants

Simple methods for projecting low-cost routes from an existing road network to target locations (i.e. harvest cutblocks) include simple least cost paths (LCP), dynamic least cost paths (DLCP) and least cost path minimum spanning trees (MST) \@ref(fig:fig2)). We use Dijkstra's shortest path algorithm [-@dijkstra_note_1959] to determine the least cost path between a target location (e.g. centroid of a cutblock) and the existing road network. Least cost paths to each cutblock is determined independently in simple method (LCP). The dynamic least cost paths (DLCP) method evaluates the path to each cutblock in sequence so that earlier paths are considered in the cost for later paths. Sequence is determined by a heuristic such as the distance to the nearest existing road in these examples. The minimum spanning tree (MST) method calculates the minimum cost network to connecting all cutblocks to the road network using Kruskal's algorithm [-@kruskal_shortest_1956]. See Appendix A for a more detailed explanation of these methods. We focus our analysis on the DLCP and MST approaches which yield more realistic road networks (Figure \@ref(fig:fig2)). We compared these methods to an alternative iterative least cost path method developed by Hardy [-@hardy_forest_2019].  
...
To make help make these road network projection methods more easily and widely applicable, we implemented the simple least cost path, dynamic least cost path and minimum spanning tree methods in the open source R package 'roads'. 
...
<!-- TO DO: update the details on getDistFromSource function - where is it, or where should it be? -->
...
<!-- TO DO: consider breaking down aspatial results into 'cutblocks', 'outside cutblocks' and 'overall': it may make the results easier to explain and interpret. -->
...
```{r fig3, fig.height=3, fig.width=fig_widths["two"], fig.cap="Proportional difference of projected mean from observed mean road metrics using different projection method variants with a 1 ha resolution cost raster across the entire Revelstoke Timber Supply Area (“overall”) and within cutblocks (“cutblocks”). Values greater than 0 indicate overprojection of the metric and negative values indicate an underprojection. The cutblocks only method is not defined for the road density, road presence or distance to road characteristics."}
...
- as expected, not great at projecting exact road locations. sources of discrepancy include switchback, stream crossings, inaccurate harvest locations. Missing switchbacks and too many stream crossings also reduce our total amount of roads. All of these could be improved with more detailed high resolution cost & harvest and landing data. 
...
<!-- TO DO: switch fig resolution labels from metres to ha. -->
```{r get-data2}
...
```{r figA1, fig.height=10, fig.width=fig_widths["two"], fig.cap="Proportional difference of projected mean from observed mean road metrics using different projection methods across the entire Revelstoke Timber Supply Area (“overall”) and within cutblocks (“cutblocks”) for the fine resolutiom (1 ha) and coarse resolution (100 ha) cost rasters. Values greater than 0 indicate the projection overestimated the metric and negative values indicate an underestimation."}
# organize data for ggplot
meanTable_long <- meanTable %>% dplyr::select(-runTime) %>% 
