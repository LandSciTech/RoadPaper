...
In addition to the projection methods included in the `roads` package we also applied the Forest Road Network Plugin for QGIS by Clément Hardy (<https://github.com/Klemet/ForestRoadNetworkPluginForQGIS>). This plugin uses a similar DLCP algorithm to the `roads` package and uses the distance to the closest roads to order the cutblocks.

## Analysis

We projected road development in the Revelstoke TSA from 1990 to the present using several different inputs and compared the projections to the observed present day road network in order to evaluate the accuracy of the projections and their sensitivity to different inputs. We compared two resolutions for the cost surface by aggregating the fine resolution cost raster to a coarser 1000 m resolution. We also compared methods for locating landing points within cutblocks by comparing using the polygon centroid and regularly spaced and random points, with point densities of 1 and 10 pts/km^2^ (Figure \@ref(fig:fig2)). For the Hardy QGIS plugind supplied our cost surface, cutblocks, road network data and kept all the default settings except for the skidding distance which was set to 200m. Note that a skidding distance of 200 m corresponds to \~ 9 pts/km^2^ .
...
pal <- met.brewer(pal_nm, 8) %>% .[c(1,3,5,7)]
...
allMaps <- purrr::map(
  ...
  ~ qtm(tsaCost, raster.style = "cont", raster.palette = "Greys",
        raster.alpha = 0.5)+
    qtm(cutblocks, fill = NULL, borders = pal[1], borders.lwd = 1.5)+
    qtm(roads, lines.col = pal[3], lines.lwd = 2)+
    qtm(read_sf(here(allProj$output[[.x]])), lines.col = pal[2], lines.lwd = 2)+
...
```{r fig2, fig.cap="Example of the different projection methods and sampling strategies. Target cutblocks are shown by grey lines, existing roads by black lines, projected roads by green lines, observed roads by red lines, and the cost of road building by the grey shading."}
...
```{r fig3, fig.height=4.5, fig.width=fig_widths["mid"], fig.cap="Proportional difference of projected mean from observed mean road metrics using different projection methods across the entire Revelstoke TSA (“overall”) and within cutblocks (“cutblocks”). Values greater than 0 indicate the projection overestimated the metric and negative values indicate an underestimation."}
...
prop_dif %>% 
  ...
  theme_classic()+
  scale_fill_manual(values = met.brewer(pal_nm, 8) %>% .[c(8, 1:6)],
                    name = "Sampling\nMethod", guide = "none")+
...
```{r ftNelson, fig.height=6, fig.width=fig_widths["two"], fig.cap="Road projections for the Fort Nelson Timber Supply Area. Black lines show projected and pre-existing roads. Red lines show observed roads that were not captured by the projections."}
ftN_roads_proj <- read_sf(here("analysis/data/derived_data/TSA8_1000/centroid_1e-05.shp"))

ftN_roads_obs <- read_sf(here("analysis/data/derived_data/combined_ft_nelson_roads.gpkg"))

qtm(ftN_roads_obs, lines.col = "red")+
  qtm(ftN_roads_proj)
```
