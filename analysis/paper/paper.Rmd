...
To spatially define the cost of constructing and traveling on a forestry road we developed a cost raster. We used several spatial raster layers to construct our cost surface. Following the Interior Appraisal Manual of BC, the base cost per kilometer of road was a linear function of slope [@governmentofbritishcolumbia2021a]. Separate costs for river and pipeline crossings were then added and major water bodies were considered barriers and assigned a very high cost. Lastly, existing roads were assigned a cost of 0 (Table 1). The cost raster produced consists of 1703936 pixels with a resolution of 100 meters by 100 meters.

Table 1. Definitions of road building costs used to build the cost raster.
...
We developed the `roads` R package to project where new roads will be built based on the existing road network, locations of expected resource extraction (i.e. harvest cutblocks) and the landscape connecting the two. There are several different methods available for solving this problem, including snapping, least cost paths, dynamic least cost paths and minimum spanning trees (Figure \@ref(fig:fig2)). Snapping follows a straight line between the existing road and each cutblock regardless of the intervening landscape. The least cost paths (LCP) method uses Dijkstra's shortest path algorithm [-@dijkstra1959] to determine the optimal path between a cutblock and the existing road based on the cost of building a road at any point between the two. The path to each cutblock is determined independently of the other cutblocks. The dynamic least cost paths (DLCP) method uses the same LCP algorithm but evaluates the path to each cutblock in sequence so that earlier paths are considered in the cost for later paths. The sequence is determined by a heuristic such as the distance to the existing road. Finally, the minimum spanning tree (MST) method builds on the LCP method by determining the least cost path between all cutblocks and the road and between each cutblock. Then the minimum spanning tree connecting all cutblocks to the road is solved using Kruskal's algorithm [-@kruskal1956]. (See Appendix X for a more detailed explanation of these methods). In the remainder of this paper, we focus on the DLCP and MST approaches which are better aligned with how road networks are designed. In addition to the projection methods included in the `roads` package, we also applied the Forest Road Network Plugin for QGIS by Clément Hardy (<https://github.com/Klemet/ForestRoadNetworkPluginForQGIS>), hereafter referred to as the Hardy QGIS method. This plugin uses a similar DLCP algorithm to the `roads` package and uses the distance to the closest roads to order the cutblocks. Finally, in some cases the impact of forestry on wildlife is projected without considering the road development that will accompany it, we refer to this as the cutblocks only projection method and compare it to projections that do consider roads.
